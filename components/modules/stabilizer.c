...
static void stabilizerTask(void* param)
{
  ...
  DEBUG_PRINTD("Wait for sensor calibration...\n");
  ...
  DEBUG_PRINTD("stabilizerTask RUNNING .\n");
  ...
  while(1) {
    ...
    if (testState != testDone) {//默认不测量
      ...
      checkEmergencyStopTimeout();//TODO:disable
...
static void testProps(sensorData_t *sensors)
{
  ...
  else if (testState == measureProp)
  {
    ...
    else if (i == PROPTEST_NBR_OF_VARIANCE_VALUES)
    {
      ...
      DEBUG_PRINTD("Motor M%d variance X+Y:%f (Z:%f)\n",
...
//      DEBUG_PRINTD("IdleV: %f, minV: %f, M1V: %f, M2V: %f, M3V: %f, M4V: %f\n", (double)idleVoltage,
