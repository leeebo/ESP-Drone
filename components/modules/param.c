...
void paramTOCProcess(int command)
{
  ...
  uint16_t n=0;
  uint16_t paramId=0;
  DEBUG_PRINTD("paramTOCProcess command = %d!\n",command);
  switch (command)
  {
  case CMD_GET_INFO: //Get info packet about the param implementation
    DEBUG_PRINTD("Client uses old param API!\n");
    ptr = 0;
    group = "";
...
static void paramWriteProcess()
{
  if (useV2) {
    ...
    DEBUG_PRINTD("paramWriteProcess \n");
    int ident = p.data[0];
    ...
    if (id<0) {
  	...
  	if (params[id].type & PARAM_RONLY)
  		return;
    DEBUG_PRINTD("paramWriteProcess =%x \n",(int)valptr);
    switch (params[id].type & PARAM_BYTES_MASK)
    {
   	case PARAM_1BYTE:
   		*(uint8_t*)params[id].address = *(uint8_t*)valptr;
