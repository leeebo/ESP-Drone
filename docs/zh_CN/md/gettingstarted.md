...
* 支持自稳定模式：自动控制机身水平，保持平稳飞行
* 支持定高模式：自动控制油门输出，保持固定高度
* 支持定点模式：自动控制机身角度，保持固定空间位置
* 支持 PC 上位机调试：使用 cfclient 上位机进行静态/动态调试
* 支持 APP 控制：使用手机 APP 通过 Wi-Fi 轻松控制
* 支持游戏手柄控制：通过 cfclient 使用游戏手柄轻松控制
...
需要对 ESP-IDF 目录下的 `esp32-s2` （或esp32）组件链接脚本进行配置，目的是将 ESP=Drone 固件中带有`param`或`log`标签的变量，放置在连续的地址区域。

在 `{IDF_PATH}/components/esp32s2/ld/esp32s2.project.ld.in` 文件中找到 `drom0_0_seg` 段，并在该段段尾添加以下内容：
...
将 `Max WiFi TX power` 改为 `20`，提高 Wi-Fi 通信距离。注意提高 PHY 增益，会导致系统耗电速度加快。
